...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{imgs/eeg_headband.jpg}
\caption{EEG Headband: EEG headband with connected electrodes.}
\label{fig:eeg_headband}
\end{figure}
...
At the time of his discovery, it was already known that nerves conduct electrical signals and that these signals are used to trigger muscle tissue to contract\cite{mattucci1, matteucci2}.
...
The demonstration was succesfull with participants being able to control a mouse cursor out of a labirynth.
...
Even when the signal is standarized with respect to the reference electrode, the non-stationarity of the signal remains i.e. because of skin impedance changes at the reference electrode itself.
...
We do this by applying decomposition of EEG signal into separate components and doing classification on the components.
...
\hline
Dataset Name & Participants & Samples & Total Hours & Stimulus Type \& Setup \\
\hline
MUSIN-G & 20 & 240 & ~7.7 & 12 full songs across various genres (Indian Classical, Goth Rock, etc.). Rated for familiarity and enjoyment. \\
\hline
BCMI-Training & 10 & 2,400 & ~10.6 & Synthetic music generated in real-time. Part of a larger dataset evaluating an online BCMI system. \\
\hline
OpenMIIR & 10 & 2,400 & ~6.9 & 12 short musical fragments. Recorded during both music perception (listening) and music imagination (thinking of the song). \\
\hline
NMED-T & 20 & 200 & ~15.8 & 10 full-length Electronic Dance Music (EDM) songs. Focuses on continuous naturalistic listening and tempo. \\
\hline
ZBra / Wearable Music2Emotion & 44 & 1,760 & ~29.3 & 101 AI-generated music clips designed to elicit specific target emotions (arousal/valence). Fuses EEG and fNIRS data. \\
\hline
DEAP & 32 & 1,280 & ~21.3 & 40 one-minute music video excerpts. Widely used benchmark dataset for emotion classification (arousal, valence, dominance, liking). \\
\hline
\end{tabular}
\caption{Comparison of EEG music datasets}
\label{tab:eeg-datasets}
\end{table}
...
In studies that combine music listening with EEG collection, the music itself as an accoustic wave would be a source of noise.
...
To mitigate the above challenges, various preprocessing techniques are used to clean the data and extract relevant features \cite{cyril}.
% Some of these techniques became standardized in a form of python or matlab libraries, which are widely used in the field.
We generally consider the EEG signal to be a two-dimensional array of shape (n\_channels, n\_timepoints), where n\_channels is the number of electrodes used in the recording and n\_timepoints is the number of time points recorded for each channel, containing electrical potentials measured at each electrode over a period of time.
...
See \cite{frolich} for thorough explanation on the ICA method.
...
The classification can be done with a neural network trained on expert-labeled data\cite{iclabel}. Modern-day trained classifiers like this are a common inclusion in EEG processing libraries like ``mne\_icalabel''.

\subsection{Normalization}

There are a few reasonable ways to normalize the signal:
\begin{itemize}
  \item \textbf{Z-score normalization}: Transforms the data to have zero mean and unit variance by subtracting the mean and dividing by the standard deviation.
  \item \textbf{Min-max normalization}: Scales the data to a fixed range (typically [0, 1]) by subtracting the minimum value and dividing by the range.
  \item \textbf{Robust normalization}: Uses the median and interquartile range instead of mean and standard deviation, making it less sensitive to outliers.
\end{itemize}
