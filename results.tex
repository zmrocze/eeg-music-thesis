...
We show results on the test set.
...
We achieve the results shown in Table~\ref{tab:song-prediction} for each variant of the tested model.
For the CNN model we provide the best result out of the few tested variants, described in Figure~\ref{fig:cnn-channels-accuracy}.
...
\begin{table}[h]
  ...
  \begin{tabular}{|l|c|}
    ...
    KNN     & 0.1374 \\
    SVM     & 0.1229 \\
    XGBoost & \textbf{0.1980} \\
    CNN (best)      & 0.1802 \\
    \hline
  \end{tabular}
  \caption{Song name prediction accuracy in the MUSIN-G dataset: subject-wise split.}
  \label{tab:song-prediction}
\end{table}


Our results are comparable to the study by Pandey et al.\cite{pandey}, in which researchers achieved 12.5\% mean subject accuracy (a random classifier gives \(  \frac{1}{12} = 0.08(3) \cdot 100\% \)).

We also tested how the number of parameters influences the model's performance. We kept the architecture and varied number of convolution channels. The results are shown in Figure~\ref{fig:cnn-channels-accuracy}.
...
As the predictions relate just a single snippet sampled from the recording, 
it is reasonable to ask what if multiple snippet predictions for snippets sampled from a single full song would be used to vote on a prediction for the full song recording.
With the assumption that the individual votes are independent, the overall accuracy should increase, apparently with no limit as we sample more and more snippets to use for voting and predicting.
In practice when we sample a lot of snippets, they start to overlap so the contributions are not independent.
But what we observe is that the accuracy does not increase or increases very slightly.
For an experiment we use the XGBoost and use it to give predictions for a 100 snippets sampled from a song and use voting to give a prediction for the whole song.
We expected the accuracy to increase significantly. Instead we observe just 20.83\% accuracy of the ensamble.
This is strong indicator that the predictions are made based on a feature repeated across all samples, which may mean it is some particular type of noise that the model focuses at.

In the study by Pandey et al.\cite{pandey}, researchers also tested scores achieved in a different testing paradigm. They split the MUSIN-G dataset into training and testing 
...
We are able to reproduce these results, and for \( n=20 \) we achieve accuracies as shown in Table~\ref{tab:song-id-temporal-split}.
...
While the best model in the trial-wise split variant achieves 85\% accuracy, the best model in subject-wise split variant achieves only 18\%.
...
The results are shown in Table~\ref{tab:song-id-single-subject}. 
For the CNN model we can't achieve reasonable accuracy. The model fails to minimize the loss function or reach any optimum. We expect the big number of labels to be the culprit.
...
For comparison the results of the other models range from 57\% to 85\% and random chance is 8\%.
This means that with a much simpler method that relies purely on simple pattern matching, we are still able to achieve results getting close to the results from more complex models.
...
% TODO: eksperyment osi. eksperyment topline spektrogram. ignored?

The BCMI-training dataset is made of computer-generated snippets of music. These musical snippets are tagged with emotionality labels, one of 9 possibilities arranged on a 3x3 Valence and Arousal (High, Medium, Low) grid.
...
Similarly to the experiment of song prediction, here we also check what happens if we let 100 snippets "vote" for prediction to the full song.
That is, we use the trained model on short snippets to classify 100 short snippets and pick the majority vote as the emotion prediction for the full song.
The accuracy of prediction in such experiment is 11.57\% which is not significantly greater than the random accuracy.
With this experiment we conclude a fail in the task of song emotion label prediction.
...
For description see Section~\ref{sec:note-onset}.
...
Results are shown in Table~\ref{tab:density-prediction}.
...
\label{sec:music-reconstruction}

We attempt to reconstruct listened-to music from the EEG signal for the MUSIN-G dataset.
Instead of reconstructing the raw waveform, the target is the mel spectrogram of the audio.
...
To judge the value of reconstructions we use a classification test.
Namely we compare the generated spectrogram (with sum squared distance) with spectrograms from the dataset, including the target one.
We then calculate AUROC score: what is the position of the target spectrogram in a list sorted by distance?
If the target spectrogram appears early in the list, it means that the generated is more similar to the target that to most other spectrograms, which can be a measure of generation quality.
The reconstructions are accurate enough to allow for meaningful classification (see Figure~\ref{fig:reconstructed-spectrograms}). 
...
See the AUROC plots in Figures~\ref{fig:auroc-median-rank} and~\ref{fig:auroc-score}.
...
To test the hypothesis that the network learns to reconstruct a time-averaged spectrogram for a music fragment, we train a simplified model.
The model has similar architecture to the main CNN model, but instead outputs an image of shape 64x1 which is then broadcasted to the desired shape of 64x64, meaning the spectrogram doesn't vary in time.
With this change we also don't need the full image decoder and therefore keep just a single fully connected layer.
An architecture diagram is shown in the methods section.
...
Comparing these results to the main model we observe few percentage points difference in AUROC scores (see Figures~\ref{fig:medianrank-timeconstant} and~\ref{fig:auroc-timeconstant}).
...
The results of full reconstruction (see Section~\ref{sec:music-reconstruction}) suggest that to decode rhytm we need to use different methods.
We take inspiration from \cite{daly} where a Bi-LSTM network is used to perform music decoding in waveform domain.
We aim to predict an onset detection function (ODF), which is a scalar function whose picks indicate sound onsets (see Section~\ref{sec:odf}).
...
Figure~\ref{fig:lstm-odf-plots} shows an example ODF prediction.
...
In all variants, eventhough we observe the loss value to decrease with training, we do not observe significant results in AUROC scores which oscillate slightly above 0\%.
Inspecting the predictions visually leaves no doubt in failure of the method.
