...
Without this step we observe the models to struggle to train.
...
For the task of detecting note onsets we use the essentia\cite{essentia} library with their HFC onset detection method to transform the raw wave audio signal into a list of note onset start times.
...
They differ in the input dimensionality they are meant to work with: one inputs EEG images after preprocessing, including band power calculation which leaves the 1s snippets to be of shape \( 60\times 10\).
The other inputs raw EEG data, which has shape \( 129\times 256\) and is used for the ablation study in the appendix. The models have 136164 and 249380 parameters respectively.
...
The reconstruction network is U-Net style with a 3 layer contracting encoder, a middle bottleneck fully-connected layer, and a 3 layer expanding decoder.
The encoder is the same architecture as the classifier model, and the decoder is a matchingly a 3 layer upsampling convolutional network.
There are skip connections between matching layers of the encoder and decoder.
As the input is of different dimensionality than the output and consecutively outputs of the encoder and decoder layers are, the skip connections also apply interpolation to match the dimensions.
