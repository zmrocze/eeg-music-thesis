...
Here we consider tasks where a sing title or another song property is to be predicted from the EEG signal.
Researchers classify either full eeg recordings from listening to the whole song, just a random snippet cut from the recording or a 
specific snippet cut from the recording at a particular point in time with respect to the start of stimulus (song playing) called ERP.
...
The focus on ERPs means that every trial, which is slightly over 3 seconds long, acts as its own snippet for classification.
Therefore, the EEG snippets are aligned in time---all starting when the music began playing in the experimental scenario.
...
Even though there is some traction in tackling this challenge, the struggles and the obtained often lackluster results show that the problem is difficult.
...
Researchers attempted to answer the second question by comparing the distance matrices within the sets of extracted audio features and extracted EEG features\cite{foster} in a style of an ABX test.
The study reports both found correlation between the two representations and some although poor classification performance. They noted a need for repeated testing to get more certainty about the result.
...
This means either full decoding of the music's waveform or decoding into a spectrogram, which can be further decoded into a waveform with a vocoder.
...
The model outputs were also evaluated by human listeners using a vocoder to transform the spectrograms to waves, who were able to successfully determine which of two presented audio clips corresponded to a third target sound.
This means the model outputs were good enough to be perceived by humans, though this is not always the case here and often the model output is enough for classification but wouldn't remind of music to a human ear still.
